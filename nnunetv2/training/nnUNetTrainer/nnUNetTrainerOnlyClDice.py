...
class SoftClDiceLoss(nn.Module):
    ...
    def forward(self, x, y, loss_mask=None):
        ...
        cldc = 2.0*(tprec*tsens)/(tprec+tsens)

        cldc = cldc.mean()
        return -cldc


class clDC_loss(nn.Module):
    def __init__(self, soft_dice_kwargs, weight_cldice=1, ignore_label=None):
        """
        Weights for Dice and clDice do not need to sum to one. You can set whatever you want.
        ...
        self.cldc = SoftClDiceLoss(apply_nonlin=softmax_helper_dim1, **soft_dice_kwargs)
    ...
    def forward(self, net_output: torch.Tensor, target: torch.Tensor):
        ...
        cldc_loss = self.cldc(net_output, target_dice, loss_mask=mask) \
            if self.weight_cldice != 0 else 0

        result = self.weight_cldice * cldc_loss
        return result
    

class nnUNetTrainerOnlyClDice(nnUNetTrainer):
    @staticmethod
    ...
    def _build_loss(self):
        loss = clDC_loss({'batch_dice': self.configuration_manager.batch_dice,
                            'smooth': 1e-5, 'do_bg': False, 'ddp': self.is_ddp}, weight_cldice=1,
