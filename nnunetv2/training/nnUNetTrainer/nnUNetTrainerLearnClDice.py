...
class DC_and_clDC_loss(nn.Module):
    ...
    def forward(self, net_output: torch.Tensor, target: torch.Tensor):
        ...
        result = alpha * dc_loss + (1 - alpha) * cldc_loss
        return result
    

class nnUNetTrainerLearnClDice(nnUNetTrainer):
    @staticmethod
    ...
    def _build_loss(self):
        loss = DC_and_clDC_loss({'batch_dice': self.configuration_manager.batch_dice,
    ...
    def configure_optimizers(self):
        optimizer = torch.optim.SGD(list(self.network.parameters()) + list(self.loss.parameters()), 
